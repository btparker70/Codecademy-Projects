const menu = {
  _courses: {
    appetizers: [],
    mains: [],
    desserts: [],
  },
  get appetizers() {
    return this._courses.appetizers;
  },
  get mains() {
    return this._courses.mains;
  },
  get desserts() {
    return this._courses.desserts;
  },
  set appetizers(appetizers) {
    this._courses.appetizers = appetizers;
  },
  set mains(mains) {
    this._courses.mains = mains;
  },
  set desserts(desserts) {
    this._courses.desserts = desserts;
  },
  get courses() {
    return {
      appetizers: this.appetizers,
      mains: this.mains,
      desserts: this.desserts,
    };
  },
  addDishtoCourse(courseName, dishName, dishPrice) {
    const dish = {
      name: dishName,
      price: dishPrice,
      };
    return this._courses[courseName].push(dish);
  },
  getRandomDishFromCourse(courseName) {
    const dishes = this._courses[courseName];
    const randomIndex = Math.floor(Math.random() * dishes.length);
    return dishes[randomIndex];
  },
  generateRandomMeal() {
    const appetizer = this.getRandomDishFromCourse('appetizers');
    const main = this.getRandomDishFromCourse('mains');
    const dessert = this.getRandomDishFromCourse('desserts');
    const total = (appetizer.price) + (main.price) + (dessert.price);
    return `Your three course meal is ${appetizer.name}, ${main.name} and ${dessert.name} for ${total} dollars.`;
  }  
};

menu.addDishtoCourse('appetizers', 'salad', 8.00);
menu.addDishtoCourse('appetizers', 'bread', 5.00);
menu.addDishtoCourse('appetizers', 'soup', 7.00);
menu.addDishtoCourse('mains', 'steak', 24.00);
menu.addDishtoCourse('mains', 'pasta', 16.00);
menu.addDishtoCourse('mains', 'fish', 20.00);
menu.addDishtoCourse('desserts', 'cake', 8.00);
menu.addDishtoCourse('desserts', 'espresso', 4.00);
menu.addDishtoCourse('desserts', 'gellato', 6.00);

const meal = menu.generateRandomMeal();
console.log(meal);
